...
use candid::parser::typing::{check_file, check_prog, check_file_with_options, CheckFileOptions, TypeEnv};
use goldenfile::Mint;
use std::io::Write;
use std::path::Path;
...
fn test_combine_actors() {
    ...
    match check_file_with_options(
        ...
        Ok(result) => {
            {
                let mut output = mint.new_goldenfile(filename.with_extension("did")).unwrap();
                let content = candid_export::compile(&result.types, &result.actor);
                // Type check output
                let ast = content.parse::<IDLProg>().unwrap();
                check_prog(&mut TypeEnv::new(), &ast).unwrap();
                writeln!(output, "{}", content).unwrap();
